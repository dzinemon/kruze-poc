  ...
  return (
    <section className="py-16 px-6 bg-bg-base">
      <div className="max-w-6xl mx-auto">
        <p className="text-xs text-text-muted mb-8">mediaAndTextBlock — image {section.imagePosition ?? "left"}</p>
        <div className={`flex flex-col lg:flex-row gap-12 items-center ${isImageLeft ? "" : "lg:flex-row-reverse"}`}>
          {section.image?.asset && (
            <div className="flex-1">
              ...
              />
              {section.caption && (
                <p className="text-sm text-text-muted mt-2">{section.caption}</p>
              )}
            </div>
          )}
          <div className="flex-1">
            {!section.image?.asset && (
              <div className="rounded-md bg-bg-subtle border border-border-subtle h-48 flex items-center justify-center text-text-muted">
                No image
              </div>
            )}
          </div>
